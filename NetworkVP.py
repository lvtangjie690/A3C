...
class NetworkVP(object):
    ...
    def _create_graph(self):
        ...
        self.opt_grad_all = self.opt.compute_gradients(self.cost_all)
        grad_vars = [v for (g, v) in self.opt_grad_all if g is not None]
        self.opt_grad = self.opt.compute_gradients(self.cost_all, grad_vars)

        if Config.USE_GRAD_CLIP:
            self.opt_grad = [(tf.clip_by_average_norm(g, Config.GRAD_CLIP_NORM), v) for g, v in self.opt_grad]
        
        self.grad_holder = [(tf.placeholder(tf.float32, shape=g.get_shape()), v) for (g, v) in self.opt_grad]
        self.train_op = self.opt.apply_gradients(self.grad_holder)
